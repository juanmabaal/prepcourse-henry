...
function pasarUsuarioAPremium(objetoMuchosUsuarios) {
   ...
   for (let i = 0; i < objetoMuchosUsuarios.length; i++) {
      for(var prop in objetoMuchosUsuarios[i]){
         if(prop === "esPremium") {
            objetoMuchosUsuarios[i][prop] = true;
         }
      }
...
function sumarLikesDeUsuario(objetoUsuario) {
   ...
   // Tu código:
   //var objetoUsuario = {posts: [{post: {likes: 1}, post:{likes:2}}]} => ejemplo
   let contador = 0;
   for(var postsIndex in objetoUsuario.posts) {
      for (var postIndex in objetoUsuario.posts[postsIndex]) {
         if(postIndex === "likes"){
            contador += objetoUsuario.posts[postsIndex][postIndex]
         }
